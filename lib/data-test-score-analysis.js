...
module.exports = function(data) {
  ...
  _.each(districts, d => {
    ...
    aggregateDistrict.beatingOddsM = _.filter(schools, s => {
      let fy = _.find(s.years, { year: TEST_RECENT_YEAR });
      let fs = fy ? _.find(fy.subjects, { subject: 'm' }) : null;
      return fy && fs ? fs.oddsCategory === '3' : false;
    }).length;
    aggregateDistrict.beatingOddsR = _.filter(schools, s => {
      let fy = _.find(s.years, { year: TEST_RECENT_YEAR });
  ...
  return _.sortBy(allSchools, 'name');
};
